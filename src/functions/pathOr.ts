...
import { type GetPathValue } from './path';
import { curry } from './curry';

function pathOrImpl<T, const P extends ArrayContainer<PropertyKey>, D>(defaultValue: D, path: P, obj: T): GetPathValue<T, P, D> {
  if (path.length === 0) return obj as GetPathValue<T, P, D>;
  if (obj == null) return defaultValue as GetPathValue<T, P, D>;

  const [head, ...tail] = path;
  if (!(head in (obj as object))) return defaultValue as GetPathValue<T, P, D>;

  const value = (obj as Record<PropertyKey, unknown>)[head];
  return pathOrImpl(defaultValue, tail, value) as GetPathValue<T, P, D>;
}
