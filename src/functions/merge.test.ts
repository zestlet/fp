...
import { mergeLeft, mergeLeftInPlace, mergeDeleteSymbol } from './merge';
...
describe('merge', () => {
  ...
  it('应该正确合并两个简单对象', () => {
    ...
    const result = mergeLeft(target, source);
  ...
  describe('mergeInPlace', () => {
    it('应该支持原地修改', () => {
      ...
      const source = { b: { y: 2 } };
      const result = mergeLeftInPlace(target, source);
  ...
  it('应该正确处理无效输入', () => {
    // @ts-ignore
    expect(mergeLeft(null, { a: 1 })).toEqual({ a: 1 });
    // @ts-ignore
    expect(mergeLeft({ a: 1 }, null)).toEqual(null);
    // @ts-ignore
    expect(mergeLeft(undefined, { b: 2 })).toEqual({ b: 2 });
    // @ts-ignore
    expect(mergeLeft({ b: 2 }, undefined)).toEqual(undefined);
  });
