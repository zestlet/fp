...
function splitByImpl<T>(predicate: (value: T, index: number, array: ArrayContainer<T>) => boolean, array: ArrayContainer<T>): [T[], T[]] {
  const left: T[] = [];
  const right: T[] = [];
 ...
 * spiltBy(x => x % 2 === 0)(array); // [[2, 4], [1, 3, 5]]
 */
export const splitBy = curry(splitByImpl) as {
